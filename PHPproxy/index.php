...
$landingExampleURL = base64_encode("http://example.com");
...
define("PROXY_PREFIX", $_SERVER["SCRIPT_NAME"] . "?");
...
function rel2abs($rel, $base) {
  if (empty($rel)) $rel = ".";
  if (parse_url($rel, PHP_URL_SCHEME) != "" || strpos($rel, "//") === 0) return base64_encode($rel); //Return if already an absolute URL
  if ($rel[0] == "#" || $rel[0] == "?") return base64_encode($base.$rel); //Queries and anchors
  ...
  return base64_encode($scheme . "://" . $abs); //Absolute URL is ready.
}
...
if (isset($_POST["miniProxyFormAction"])) {
  $url = base64_decode($_POST["miniProxyFormAction"]);
  ...
  if (isset($queryParams["miniProxyFormAction"])) {
    ...
    $url = base64_decode($formAction . "?" . http_build_query($queryParams));
  } else {
    $url = base64_decode(substr($_SERVER["REQUEST_URI"], strlen($_SERVER["SCRIPT_NAME"]) + 1));
  }
}
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>miniProxy</title></head><body><h1>Welcome to miniProxy!</h1>miniProxy can be directly invoked like this: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />Or, you can simply enter a URL below:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + window.btoa(document.getElementById('site').value); return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Proxy It!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
...
if ($responseURL !== $url) {
  header("Location: " . PROXY_PREFIX . base64_encode($responseURL), true);
  exit(0);
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($proxifyAttributes as $attrName) {
    foreach($xpath->query("//*[@" . $attrName . "]") as $element) { //For every element with the given attribute...
      ...
      if ($attrName == "src" && preg_match("/\.mp4|\.mp3|\.flv|\.m3u8|\.avi|\.mov|\.bmp|\.mpeg|\.mpg|\.dat/i", $attrContent)) continue;
          ...
          window.XMLHttpRequest.prototype.open = function() {
              if (arguments[1] !== null && arguments[1] !== undefined) {
                ...
                if (url.indexOf("' . PROXY_PREFIX . '") == -1) {
                  url = "' . PROXY_PREFIX . '" + window.btoa(url);
                }
                arguments[1] = url;
              }
